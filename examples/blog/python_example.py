...
from generated.python.authors import (
    GetAuthorById,
    ListAuthors,
    CreateAuthor,
    UpdateAuthor,
...
from generated.python.posts import (
    GetPostById,
    ListPosts,
    CreatePost,
    PublishPost,
...
from generated.python.comments import (
    ListCommentsByPost,
    CreateComment,
    ApproveComment,
    DeleteComment,
...
from generated.python.utils import (
    set_log_level,
...
async def main():
    ...
    try:
        ...
        author_result, author_exec_time = await CreateAuthor(
            get_pool(),
            name="Sam Wilson",
        ...
        print(
            f"  Author: {author['name']} - {author['bio']} (Query time: {exec_time:.3f}s)"
        )

        # Example 4: Create a new post
        print("\n4. Creating a new post:")
        post_result, post_exec_time = await CreatePost(
            get_pool(),
        ...
        comment_result, comment_exec_time = await CreateComment(
            get_pool(),
            post_id=post_id,
        ...
        print(
            f"  Comment created (ID: {comment_result['id']}) in {comment_exec_time:.3f}s"
        )

        # Example 8: Approve the comment
        print("\n8. Approving the comment:")
        approve_result, approve_exec_time = await ApproveComment(
            get_pool(), id=comment_result["id"]
        )
        print(
            f"  Comment approved: {approve_result['approved']} in {approve_exec_time:.3f}s"
        )
        ...
        for comment in comments:
            ...
            print(
                f"  - {comment['author_name']}: {comment['content']} ({approved_status})"
            )

        # Example 10: Update the author
        print("\n10. Updating the author:")
        author_result, author_exec_time = await UpdateAuthor(
            get_pool(),
            id=author_id,
            name="Samuel Wilson",
        ...
        print(
            f"  Updated author: {author_result['name']} - {author_result['bio']} in {author_exec_time:.3f}s"
        )

        # Example 11: Update the post
        print("\n11. Updating the post:")
        post_result, post_exec_time = await UpdatePost(
            get_pool(),
            id=post_id,
        ...
        del_result, del_exec_time = await DeleteComment(
            get_pool(), id=comment_result["id"]
        )
        print(
            f"    - Deleted comment ID: {comment_result['id']} in {del_exec_time:.3f}s"
        )
